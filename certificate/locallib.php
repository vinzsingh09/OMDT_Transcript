...
function certificate_get_level($grade) {
	if($grade == ''){ //If user does not have any mark in marksheet
		$level = 'title_completion';
		return $level;
		}
	else{ //If user has mark in marksheet
		if($grade >= 0.00 && $grade < 50.00){
           $level = 'title_participation';
           return $level;

		}  
		elseif($grade >= 50.00 && $grade < 64.00){              
           $level = 'title_pass';
           return $level;

		}
		elseif($grade >= 64.00 && $grade < 78.00){
           $level = 'title_credit';
           return $level;         
        
		}
		else{
			$level = 'title_distinction';
			return $level;		   
		}
	}
}